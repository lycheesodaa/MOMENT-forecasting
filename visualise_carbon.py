...
save_path = 'figures/carbon_daily/'
data_dir = 'results/data_carbon/'
...
def load_data(_pred_len):
    ...
    df = pd.read_csv(data_dir + f'/MOMENT_Carbon_pl{_pred_len}_base_predictions.csv', index_col=0)
    if _pred_len > 12 or not using_short_horizon_forecasting:
        post_lp = pd.read_csv(data_dir + f'/MOMENT_Carbon_pl{_pred_len}_post-lp_predictions.csv', index_col=0)
    ...
    # lstm = pd.read_csv(data_dir + f'/LSTM_Carbon_pl{_pred_len}_dm200_predictions.csv', index_col=0)
    # lstm = halve_if_duplicated(lstm)
    # df['lstm'] = lstm['pred']
    #
    # # load ConvLSTM predictions into df
    # conv_lstm = pd.read_csv(data_dir + f'/ConvLSTM_Carbon_pl{_pred_len}_dm200_predictions.csv')
    # conv_lstm = halve_if_duplicated(conv_lstm)
    # df['conv_lstm'] = conv_lstm['pred']
    #
    # # load GRU predictions into df
    # gru = pd.read_csv(data_dir + f'/GRU_Carbon_pl{_pred_len}_dm200_predictions.csv')
    # gru = halve_if_duplicated(gru)
    # df['gru'] = gru['pred']
    #
    # # load GRU_Attention predictions into df
    # gru_att = pd.read_csv(data_dir + f'/GRUAttention_Carbon_pl{_pred_len}_dm200_predictions.csv')
...
def plot_multiple(df, to_exclude, title, path_to_save):
    ...
    for i, col in enumerate(columns_to_plot):
        ...
        axes[i].set_ylabel('Carbon', fontsize=14)
...
for pred_len in tqdm(predlens):
    ...
    monday_indexes = []
    mondays_temp = df.iloc[-int(len(df) * 0.2 + pred_len):-pred_len][df['date'].dt.day == 1].sample(n=2)
    monday_indexes.extend(list(mondays_temp.index))
    monday_indexes = sorted(monday_indexes)
    mondays = df.iloc[monday_indexes]
    for i, (index, monday) in enumerate(mondays.iterrows()):
        ...
        plot_multiple(sampled_week, to_exclude,
                      ...
                      save_path + f'Carbon_pl{pred_len}_predictions_week{i + 1}.png')
