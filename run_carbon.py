...
from utils.tools import log_into_csv, EarlyStopping
...
parser.add_argument('--feats_pct', type=int, default=100, help='specify top % correlated features, default all feats')
...
if args.feats_pct != 100:
    args.des += f'_{args.feats_pct}'
...
def val_or_test(loader, output_csv=False, stage=None):
    ...
    for (batch_x, batch_y, batch_x_mark, batch_y_mark, input_mask) in tqdm(loader, total=len(loader)):
        ...
        all_mse.append(criterion(pred, true))
        all_mae.append(mae_metric(pred, true))
    ...
    if output_csv:
        ...
        all_preds = torch.cat(all_preds).cpu().float().numpy()
        all_true = torch.cat(all_true).cpu().float().numpy()
        ...
        df.to_csv(args.results_path + f'MOMENT_{args.data}_ctx{args.seq_len}_pl{args.pred_len}_{stage}_predictions.csv')
...
if args.feats_pct == 0:
    # Zero-shot evaluation if univariate -> model is channel-indep., so its useless to run multivariate zero-shot
    model.eval()
    with torch.no_grad():
...
for run in range(args.train_epochs):
    losses = []
    ...
    model.eval()
    with torch.no_grad():
        val_loss, val_mae_loss = val_or_test(vali_loader, False)
    model.train()

    print(f"[INFO]: Epoch {run + 1}: Train MSE loss: {average_loss:.3f} Val MSE loss: {val_loss}")
